...
export function initializeSocket(displayName) {
    ...
    const socket = connectChat({ reconnection: false });
...
function showFriendRequestToast(fromUsername, requestId) {
    // Le pseudo vient du socket de chat, qui ne vérifie pas le JWT du handshake :
    // il est entièrement contrôlé par l'émetteur et doit être échappé.
    ...
    container.innerHTML = html`
        <div class="toast" role="alert" aria-live="assertive" aria-atomic="true">
            ...
            <div class="toast-body">
                ...
                <button class="btn btn-primary accept-friend-request">Accept</button>
                <button class="btn btn-secondary reject-friend-request">Reject</button>
            </div>
        </div>
    `;

    // Les anciens `onclick="acceptFriendRequest(...)"` cherchaient ces fonctions
    // sur `window` alors qu'elles sont importées : les boutons ne faisaient rien.
    const toastElement = container.querySelector('.toast');
    const toast = new bootstrap.Toast(toastElement);
    const close = () => { toast.hide(); container.remove(); };

    container.querySelector('.accept-friend-request')
        .addEventListener('click', () => { acceptFriendRequest(requestId); close(); });
    container.querySelector('.reject-friend-request')
        .addEventListener('click', () => { rejectFriendRequest(requestId); close(); });

    // `document.body.innerHTML +=` reconstruisait tout le DOM et faisait sauter
    // chaque écouteur déjà posé sur la page.
