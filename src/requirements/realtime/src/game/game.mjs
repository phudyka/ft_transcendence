...
function IApad(pad2, ball) {
    ...
    const distance = Math.hypot(ball.position.x - pad2.position.x,
                                ball.position.z - pad2.position.z);

    if (distance < 1.50) {
        const deltaY = ball.position.z - pad2.position.z;
        let newY = pad2.position.z + deltaY * speed;
        ...
        pad2.position.z = newY;
    } else {
        const imprecise = (Math.random() - 0.5) * 0.4;
        const targetPosition = ball.position.z + imprecise;
        const deltaY = targetPosition - pad2.position.z;
        let newY = pad2.position.z + deltaY * impreciseSpeed;
...
function checkWallCollision(ball, pad1, pad2, io, room, roomsTypes, players, roomTeams = null) {
    ...
    if (ball.position.z + ball.direction.z * ball.speed > tableHeight / 2 - ball.radius - 0.02) {
        ball.direction.z *= -1;
        ball.position.z = tableHeight / 2 - ball.radius - 0.02;
    } else if (ball.position.z + ball.direction.z * ball.speed < -tableHeight / 2 + ball.radius + 0.02) {
        ball.direction.z *= -1;
        ball.position.z = -tableHeight / 2 + ball.radius + 0.02;
    }
    if (ball.position.x > tableWidth / 2 + ball.radius) {
        ball.resetPosition();
        pad1.score++;
...
function Movepad(pad1, pad2, keysPressed, room, io, pad3, pad4) {
    ...
    const padLimit = tableHeight / 2 - padHeight / 2;
            ...
            if (keysPressed.pad1MoveDown && pad1.position.z + pad1.speed < padLimit) {
                pad1.position.z += pad1.speed;
            }
            if (keysPressed.pad1MoveUp && pad1.position.z - pad1.speed > -padLimit) {
                pad1.position.z -= pad1.speed;
            }
            if (keysPressed.pad2MoveDown && pad2.position.z + pad2.speed < padLimit) {
                pad2.position.z += pad2.speed;
            }
            if (keysPressed.pad2MoveUp && pad2.position.z - pad2.speed > -padLimit) {
                pad2.position.z -= pad2.speed;
            }
            if (pad4){
                if (keysPressed.pad3MoveDown && pad3.position.z + pad3.speed < padLimit) {
                    pad3.position.z += pad3.speed;
                }
                if (keysPressed.pad3MoveUp && pad3.position.z - pad3.speed > -padLimit) {
                    pad3.position.z -= pad3.speed;
                }
                if (keysPressed.pad4MoveDown && pad4.position.z + pad4.speed < padLimit) {
                    pad4.position.z += pad4.speed;
                }
                if (keysPressed.pad4MoveUp && pad4.position.z - pad4.speed > -padLimit) {
                    pad4.position.z -= pad4.speed;
                }
            }
            ...
            const pads = pad4 ? [pad1, pad2, pad3, pad4] : [pad1, pad2];
            ...
            if (pad4)
            {
                io.in(room).emit('movePad', {
                    pad1: pad1.position.z,
                    pad2: pad2.position.z,
                    pad3: pad3.position.z,
                    pad4: pad4.position.z
                });   
            } else {
                io.in(room).emit('movePad', {
                    pad1: pad1.position.z,
