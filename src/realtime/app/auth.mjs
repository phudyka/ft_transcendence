...
if (!SECRET) {
  throw new Error(
    "DJANGO_SECRET_KEY manquant : impossible de vérifier les jetons.",
  );
}
...
// Variante réservée au namespace /game : un visiteur sans compte est admis,
// mais en invité. La page d'accueil monte le jeu avant toute inscription — la
// scène 3D est l'argument du produit et elle était jusqu'ici enfermée derrière
// un formulaire.
//
// L'invité n'a que le solo contre l'IA : `sockets.mjs` ne lui installe aucun
// gestionnaire multijoueur, tournoi ou invitation, il n'entre jamais dans la
// table des joueurs adressables, et le namespace /chat reste sous `requireAuth`.
// Sans compte, il n'y a rien à usurper et aucune statistique à écrire.
export function allowGuest(socket, next) {
  const token = socket.handshake.auth?.token || socket.handshake.query?.token;
  const user = verifyAccessToken(token);
  if (user) {
    socket.data.user = user;
  ...
  socket.data.user = {
    id: null,
    displayName: `Guest-${socket.id.slice(0, 6)}`,
    guest: true,
  };
