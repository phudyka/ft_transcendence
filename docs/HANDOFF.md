...
Point d'entrée pour reprendre le travail. `PLAN.md` reste la référence de fond
(architecture cible, lots, journal détaillé des décisions).
...
Tout est commité sur `remaster` (`main` intact) et la pile tourne. Le premier
essai en navigateur a eu lieu le 20 août 2026 : c'est fait, il reste
l'interface.
...
Deux comptes d'essai existent en base : `alice` / `bob`, mot de passe
`pong1234`.
...
- le handshake socket.io aboutit sur `/game` **et** `/chat` en WSS à travers
  nginx, avec un jeton réellement émis par Django.
...
Il a fallu une correction pour y arriver : `main.js` charge le routeur derrière
un `await import()`, et l'évaluation d'un module à `await` de premier niveau se
termine **après** `DOMContentLoaded`. `app.js` posait son écouteur sur un
événement déjà passé, `initRouter()` ne partait jamais et la page restait
blanche, sans erreur en console. Le démarrage teste désormais
`document.readyState`.

**Seul chemin jamais exercé : le retour de l'OAuth 42.** La clé de l'application
sur l'intra est périmée et l'application déclarée pointe encore vers
...
Deux failles connues et non corrigées vivent dans ce chemin mort. **À réparer
avant de le rallumer**, pas après :

- `auth_42_login` n'envoie pas de paramètre `state`, et `auth_42_callback` n'en
  vérifie aucun : rien n'empêche un attaquant de faire consommer son propre code
  d'autorisation à la victime (CSRF OAuth). Correctif : un
  `secrets.token_urlsafe(32)` posé en session avant la redirection, comparé au
  retour, échec fermé si écart.
- `auth_42_callback` renvoie les jetons d'accès et de rafraîchissement **dans la
  chaîne de requête** de la redirection vers le frontend. Ils atterrissent dans
  l'historique du navigateur, les journaux du proxy et l'en-tête `Referer`.
  Correctif : un code à usage unique échangé par POST contre les jetons, ou des
  cookies `HttpOnly Secure SameSite=Lax`, plus un `Referrer-Policy` strict sur
  la réponse.
...
- `src/.env` était resté dans sa forme d'avant le remaster (variables
  ...
  Django neuve de 48 octets — l'ancienne faisait 23 octets, sous le minimum
  HS256. `make update-hostname` a disparu avec elle : plus aucun hôte n'est en
  dur.
...
- le client secret OAuth 42 — déjà périmé, donc sans danger, mais une clé neuve
  sera nécessaire pour rouvrir ce chemin
...
part et d'autre de l'écran envoient désormais les mêmes touches, et
n'apparaissent que sur pointeur grossier
(`@media (hover: none) and (pointer: coarse)`). Elles ne s'affichent que du côté
réellement piloté, ce qui dépend du mode — d'où
...
- `#space` (« Press space to Launch the game ») posait un écouteur `keydown` à
  ...
  affichée pendant la partie. L'invite sert maintenant de bouton, faute de
  clavier sur mobile ;
- les cinq surfaces censées recevoir l'ombre dans `loadIsland.mjs` étaient
  testées derrière `child.isMesh`, or trois d'entre elles ne sont pas des mesh :
  leur géométrie est un enfant anonyme. Seuls les deux socles ont jamais reçu
  d'ombre. Contrairement à ce qui était noté ici, les noms étaient bons —
  `three` assainit les noms au chargement, espaces en tirets bas et points
  retirés, ce que le code d'origine reflétait fidèlement. `check-assets.mjs`
  échoue désormais si un de ces noms disparaît du GLB ;
...
selon qu'il vient de l'inscription, de l'OAuth 42 ou d'un envoi de fichier. Le
client rattrape les deux formes au coup par coup. Ce champ n'est volontairement
pas validé côté serveur — l'échappement suffit à fermer la faille XSS, et durcir
maintenant casserait des comptes existants.
