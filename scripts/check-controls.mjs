...
const cases = [
    ...
    ['solo, gauche vers le haut',   { controlledPad: 1 }, 'w',         { pad: 1, direction: 'up' }],
    ['solo, gauche vers le bas',    { controlledPad: 1 }, 's',         { pad: 1, direction: 'down' }],
    ['solo, flèches inertes',       { controlledPad: 1 }, 'ArrowUp',   null],
    ['en ligne côté droit',         { controlledPad: 2 }, 'ArrowDown', { pad: 2, direction: 'down' }],
    ['en ligne côté droit, w inerte', { controlledPad: 2 }, 'w',       null],
    ['à quatre, pad 3 à gauche',    { controlledPad: 3 }, 's',         { pad: 3, direction: 'down' }],
    ['à quatre, pad 4 à droite',    { controlledPad: 4 }, 'ArrowUp',   { pad: 4, direction: 'up' }],
    ['local à deux, gauche',        { controlledPads: [1, 2] }, 'w',       { pad: 1, direction: 'up' }],
    ['local à deux, droite',        { controlledPads: [1, 2] }, 'ArrowUp', { pad: 2, direction: 'up' }],
    ['avant attribution',           { controlledPad: 0, controlledPads: 0 }, 'w', null],
    ['état initial',                { controlledPad: null, controlledPads: null }, 'ArrowUp', null],
...
assert.equal(padForSide('left', { controlledPad: 2 }), null, 'zone gauche masquée côté droit');
assert.equal(padForSide('right', { controlledPad: 4 }), 4, 'zone droite affichée pour le pad 4');
assert.equal(padForSide('right', { controlledPads: [1, 2] }), 2, 'zone droite en local à deux');
